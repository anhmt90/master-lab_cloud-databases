...
public class ECSAppTest extends TestCase {
	...
	public void testIsValidArgs() throws NoSuchMethodException {
		...
		String[] invalidInitArgs = {"initiate", "invalidArgs"};
		String[] validInitArgs = {"initiate", "5 600 FIFO"};
		String[] invalidAddArgs = {"add", "invalidArgs"};
		String[] validAddArgs = {"add", "600 FIFO"};
		try {
			validInit = (boolean) isValidArgs.invoke(testApp, "initiate", validInitArgs);
			invalidInit = (boolean) isValidArgs.invoke(testApp, "initiate", invalidInitArgs);
