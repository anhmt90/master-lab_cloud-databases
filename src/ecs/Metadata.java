...
public class Metadata implements Serializable {
    static List<NodeInfo> meta = new ArrayList<>();
...
//        System.out.println(String.format("add RANGE %s %s", start, end));
    ...
    public static int getIndexById(String nodeId) {
        for (int i = 0; i < meta.size(); i++) {
    ...
    public static int getIndexByKeyResponsibility(String keyHashed) {
        for (int i = 0; i < meta.size(); i++) {
            NodeInfo nodeInfo = meta.get(i);
            if (nodeInfo.getRange().contains(keyHashed)) {
                return i;
    ...
    public static NodeInfo get(int index) {
